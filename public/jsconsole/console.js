...
function run(cmd) {
	...
	if (internalCmd) {
		window.result('info', cleanse(stringify(internalCmd)));
	} else {
		// Send the command across the wire.
		sendMessage(cmd);
	} 
}
...
window.result = function(className, result) {
    ...
    if (className != 'info') prettyPrint([span]);
    el.appendChild(span);
...
function showhelp() {    
    // JOHN
    var src = location.protocol + '//' + location.host + '/debug/' + channelName + '.js';	
    var test = location.protocol + '//' + location.host + '/test/' + channelName + '.html';	
    
    var commands = [
    	'Mr.Toad helps debug webapps on mobile devices!',
    	'Paste this script debug script into a webpage:',
    	'',
        '<script src=\'' + src + '\'></script>',
        '',
        'Then browse there with your mobile browser.',
        'Presto! Remote debugging!',
        '',
        'Or browse directly to this test page:',
        '',
        test
    ];
	//  var commands = [
...
var exec = document.getElementById('exec'),
    ...
    // JOHN: not working across the wire yet.
    enableCC = false;
